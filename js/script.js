...
async function loadArticles() {
    try {
        ...
        const platformOrder = ['substack', 'gfg', 'dev'];
        const platformNames = {
            'substack': 'Substack Articles',
            'gfg': 'GeeksforGeeks Articles',
            'dev': 'DEV.to Articles'
        };
        
        const groupedArticles = {};
        articles.forEach(article => {
            const platform = article.platformShort;
            if (!groupedArticles[platform]) {
                groupedArticles[platform] = [];
            }
            groupedArticles[platform].push(article);
        });
        ...
        const substackIcon = `<i class="fa fa-newspaper" style="font-size: 18px;"></i>`;
        
        const getPlatformIcon = (platformClass) => {
            if (platformClass === 'dev') return devIcon;
            if (platformClass === 'gfg') return gfgIcon;
            if (platformClass === 'substack') return substackIcon;
            return '';
        };
        
        // Render articles grouped by platform
        platformOrder.forEach(platform => {
            if (groupedArticles[platform] && groupedArticles[platform].length > 0) {
                // Add section header
                const sectionHeader = document.createElement('div');
                sectionHeader.className = 'articles-section-header padd-15';
                
                // Add subscribe link for Substack
                if (platform === 'substack') {
                    sectionHeader.innerHTML = `
                        <div class="section-header-content">
                            <h3>${platformNames[platform]}</h3>
                            <a href="https://aryanshourie.substack.com/?r=215xfb&utm_campaign=pub-share-checklist" target="_blank" rel="noopener noreferrer" class="subscribe-btn">
                                <i class="fa fa-envelope"></i> Subscribe to Frontend Engineering Weekly
                            </a>
                        </div>
                    `;
                } else {
                    sectionHeader.innerHTML = `<h3>${platformNames[platform]}</h3>`;
                }
                articlesContainer.appendChild(sectionHeader);
                
                // Add articles for this platform
                groupedArticles[platform].forEach(article => {
                    const articleCard = document.createElement('div');
                    articleCard.className = 'article-item padd-15';
                    
                    const platformClass = article.platformShort;
                    const platformIcon = getPlatformIcon(platformClass);
